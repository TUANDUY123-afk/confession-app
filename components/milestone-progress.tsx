...
// 🌸 Các mốc kỷ niệm tình yêu (chỉ mốc lớn - hàng trăm ngày)
const milestones = [
...
export default function MilestoneProgress({ currentDays }: MilestoneProgressProps) {
  ...
  // ✅ Nếu đã qua hết các mốc
  if (!next) {
    return (
      <motion.div
        className="text-center text-pink-600 font-medium mt-6"
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
      >
        🌹 Hai bạn đã cùng nhau đi qua hơn {currentDays} ngày rồi 💞
        <br />
        Cùng hướng tới những kỷ niệm ngọt ngào tiếp theo nhé ✨
      </motion.div>
    )
  }

  const progress =
    prev && next ? ((currentDays - prev.days) / (next.days - prev.days)) * 100 : (currentDays / next.days) * 100

  const remainingDays = next.days - currentDays
  ...
  return (
    <div className="mb-10">
      ...
      <p className="text-center text-xs text-gray-500 mb-6 italic animate-fade-in">
        Còn {remainingDays} ngày nữa đến {next.label} 💖
      </p>

      {/* 💞 Timeline ngang cuộn được bằng tay (cực mượt mobile) */}
      <div
        className="relative overflow-x-auto scrollbar-hide touch-pan-x select-none"
        style={{
          WebkitOverflowScrolling: "touch", // mượt trên iOS
        }}
      >
        <div className="flex items-center gap-6 min-w-max px-3 py-2 relative">
          {/* Đường nối chính */}
          <motion.div
            ...
            return (
              ...
              >
                <motion.div
                  ...
                  transition={reached ? { repeat: Number.POSITIVE_INFINITY, duration: 2.5 } : {}}
                >
                  {reached ? (
